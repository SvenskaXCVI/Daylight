...
wss.on('connection',socket=>{
  socket.on('message',raw=>{try{const message=JSON.parse(raw);const code=String(message.pairingCode||'').trim().toUpperCase();if(!code||code.length<6)return send(socket,{type:'error',message:'A valid pairing code is required.'});if(socket.room&&socket.room!==code)rooms.get(socket.room)?.delete(socket);socket.room=code;room(code).add(socket);if(message.type==='hello'){send(socket,{type:'ready',devices:room(code).size});for(const peer of room(code))if(peer!==socket)send(peer,{type:'sync-request',pairingCode:code,deviceId:message.deviceId})}if(message.type==='snapshot'){for(const peer of room(code))if(peer!==socket)send(peer,message)}}catch{send(socket,{type:'error',message:'Invalid message.'})}});
